...
public class App {
	...
	private static void readJson() {
        Gson gson = new Gson();
        try (Reader reader = new FileReader(OUTPUT_JSONS + today.minusDays(1).format(DateTimeFormatter.ISO_LOCAL_DATE), StandardCharsets.UTF_8)) {
	...
	private static void writeJson() {
        ...
        try (FileWriter writer = new FileWriter(OUTPUT_JSONS + today.format(DateTimeFormatter.ISO_LOCAL_DATE), StandardCharsets.UTF_8)) {
	...
	private static int getDailyNew(List<String> comments) {
		...
		for(String comment : comments) {
			...
			if(commentDate.equals(yesterday)) {
				dailyNew++;
//				try (FileWriter writer = new FileWriter(OUTPUT_COMMENTS + today.format(DateTimeFormatter.ISO_LOCAL_DATE), StandardCharsets.UTF_8)) {
...
//				}
    ...
    private static void writeReport() {
    	try (FileWriter writer = new FileWriter(OUTPUT_REPORTS + today.format(DateTimeFormatter.ISO_LOCAL_DATE), StandardCharsets.UTF_8)) {
